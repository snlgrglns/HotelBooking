...
class TestimonialsController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('index','view','create','update','activate','deactivate','adminrestro','createrestro'),
				'users'=>array('@'),
			),
	...
	public function actionActivate($id){
		$test = Testimonials::model()->findByPk($id);
		if($test->saveAttributes(array('status'=>1))){
		...
		$this->redirect(array('testimonials/admin'));
	}

	public function actionDeactivate($id){
		$test = Testimonials::model()->findByPk($id);
		if($test->saveAttributes(array('status'=>0))){
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Testimonials']))
		{
			...
			$model->status = 1;
			if($model->save()) {
				$folder = Yii::app()->basePath . '/../images/testimonials/' . $model->id;
    ...
    public function actionCreaterestro(){
        $model=new Testimonials;
        ...
        if(isset($_POST['Testimonials']))
        {
            ...
            if($model->save()) {
                ...
                $this->redirect(array('adminrestro'));
            }
        }

        $this->render('createrestro',array(
            'model'=>$model,
        ));
    }
	/**
    ...
    public function actionAdminrestro()
    {
        $model=new Testimonials('search');
        $model->unsetAttributes();  // clear any default values
        $model->type='restro';
