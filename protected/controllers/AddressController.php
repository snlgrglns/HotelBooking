...
class AddressController extends Controller
{
    ...
    public function actionActivate($id){
        ...
        $prevAc = Address::model()->findAllByAttributes(array('status'=>1));
        if($address->saveAttributes(array('status'=>'1'))){
            foreach($prevAc as $pA){
                $pA->saveAttributes(array('status'=>'0'));
            }
        }
        $this->redirect(array('address/admin'));
    }
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Address']))
		{
			...
			if($model->save()){
                $actives = Address::model()->findAllByAttributes(array('status'=>1));
